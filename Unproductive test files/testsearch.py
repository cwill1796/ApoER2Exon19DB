...
print("""
<form name="Search Order" action="https://bioed.bu.edu/cgi-bin/students_20/groupG/testsearch.py" method="post">
...
# Each search form set to variables to call from
    ...
    query1 = "select affyId, goTitle, foldChange, DataInstance.symbol, gos, pathways from DataInstance join Gene using(symbol) JOIN GoMF USING(affyId) JOIN GoBP USING(affyId) JOIN GoCC USING(affyId) JOIN GOs on GoMF.goId=GOs.goId where DataInstance.affyId like '%'"
...
# first search dropdown and search box
    if search_dropdown_1 == 'gene_symbol':
        if search_box_1:
            query1 += "and symbol like '%s'"%(search_box_1)
